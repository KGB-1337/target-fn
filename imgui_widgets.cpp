...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f - 12));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 7.0f));
    ...
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_CheckMark : g.HoveredId == id ? ImGuiCol_CheckMark : ImGuiCol_CheckMark);
    RenderNavHighlight(frame_bb, id);
    ImVec2 myMin = { frame_bb.Min.x, frame_bb.Min.y + 6 };
    ImVec2 myMax = { frame_bb.Max.x, frame_bb.Max.y };
    RenderFrame(myMin, myMax, frame_col, true, 10);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, v, format);

    const ImU32 frame_col1 = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ImVec2 myMin1 = { frame_bb.Min.x, frame_bb.Min.y + 6 };
    ImVec2 myMax1 = { (grab_bb.Min.x + radius * (width - radius * 2.0f) - 48.0f), frame_bb.Max.y };
    ImVec2 xxx = { grab_bb.Min.x * (width * 2.0f) - 48.0f, grab_bb.Min.y };
    RenderFrame(myMin1, myMax1, frame_col1, true, 10);


    // Render grab
    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + radius * (width - radius * 2.0f) - 48.0f, grab_bb.Min.y + radius), radius - 1.5f + 1.0f, IM_COL32(0, 0, 0, 255));
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + radius * (width - radius * 2.0f) - 48.0f, grab_bb.Min.y + radius), radius - 1.5f, IM_COL32(255, 255, 255, 255));

    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
   // RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));

    float res = atof(value_buf);
    char buffer[50];
    sprintf(buffer, "%.1f", res);
    RenderText(ImVec2(frame_bb.Max.x - sizeof(buffer) + 15, frame_bb.Min.y + style.FramePadding.y + 10), buffer);
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y - 17), label);
